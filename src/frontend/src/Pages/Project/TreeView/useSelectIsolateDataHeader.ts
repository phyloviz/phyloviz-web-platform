...
import {parseRGB, VizLink, VizNode} from "./useTreeView";
...
export function useSelectIsolateDataHeader(
    ...
    setDoughnutChartData: React.Dispatch<React.SetStateAction<ChartData<"doughnut", (number | [number, number] | Point | BubbleDataPoint | null)[], unknown> | null>>,
    ...
    useEffect(() => {
        ...
        for (let i = 0; i < isolateDataRows.length; i++) {
            const row = isolateDataRows[i]

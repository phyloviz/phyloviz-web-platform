...
public class AccessDeserializer implements JsonDeserializer<Access> {
    @Override
    public Access deserialize(JsonElement json, Type typeOfT, JsonDeserializationContext context) throws JsonParseException {
        ...
        for (Map.Entry<String, JsonElement> entry : jsonObject.entrySet()) {
            ...
            switch (name) {
                ...
                case "api" -> details = context.deserialize(entry.getValue(), ApiAccess.class);
                case "library" -> details = context.deserialize(entry.getValue(), LibraryAccess.class);
        ...
        if (type == AccessType.LIBRARY && !(details instanceof LibraryAccess))
            throw new JsonParseException("Access details are not of type LibraryAccess even though type is LIBRARY");

        if (type == AccessType.API && !(details instanceof ApiAccess))
            throw new JsonParseException("Access details are not of type ApiAccess even though type is API");
